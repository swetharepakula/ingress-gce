...
func NewController(ctx *context.ControllerContext) *Controller {
	...
	ctx.SAInformer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(old, cur interface{}) {
			...
			if curSA.GetDeletionTimestamp() != nil {
				klog.V(4).Infof("Service Attachment %s/%s has deletion timestamp. Ignoring update", curSA.Namespace, curSA.Name)
				return
			}

			// Only process ServiceAttachments that are part of periodic requeue or have a spec change.
			if !shouldProcess(oldSA, curSA) {
				klog.V(4).Infof("Ignoring status update for ServiceAttachment %s/%s", curSA.Namespace, curSA.Name)
				return
			}
